...
class TDBKnowledgeManager {
    constructor(config = {}) {
        ...
        this.recovery = null;
        this.recoveryStartupPending = this.config.recovery !== false;
        ...
        this.activeQueueWorkerPromise = null;
        this.processedSinceFlush = 0;
        this.dirtyTextIndexLibraries = new Set();
    ...
    async initialize() {
        ...
        // Reconcile durable publications before metadata discovery, T02, ingestion or readiness.
        if (this.config.recovery !== false) {
            const { RecoveryRuntime } = require('./modules/tdbRecovery/runtime');
            this.recovery = new RecoveryRuntime(this, this.config.recovery || {});
            await this.recovery.initialize();
            this.recoveryStartupPending = false;
        }
        ...
        this._markExistingTextIndexesDirty();
    ...
    _markExistingTextIndexesDirty() {
        const libraries = this.listLibraries().map(safeLibraryName);
        for (const library of libraries) this.dirtyTextIndexLibraries.add(library);
        if (libraries.length > 0) {
            console.log(`[TDBKnowledge] 🧊 Scheduled startup text-index rebuild for ${libraries.length} existing librar${libraries.length === 1 ? 'y' : 'ies'}.`);
        }
    }
    ...
    getOrOpenLibrary(library) {
        if (this.recoveryStartupPending) throw new Error('RECOVERY_STARTUP_PENDING');
        const safeName = safeLibraryName(library);
    ...
    _enqueueIngestJob(action, filePath, options = {}) {
        if (!this.metaDb || this.isShuttingDown) return;
        const normalizedPath = this._normalizeFilePath(filePath);
    ...
    async _runQueueWorker() {
        ...
        if (this.activeQueueWorkerPromise) return this.activeQueueWorkerPromise;
        ...
        const workerPromise = (async () => {
            ...
            try {
                ...
                await this._rebuildDirtyTextIndexes('queue drain');
            } finally {
                this.isProcessing = false;
                this.isQueueWorkerRunning = false;
            }
        ...
        this.activeQueueWorkerPromise = workerPromise;
        try {
            return await workerPromise;
        } finally {
            if (this.activeQueueWorkerPromise === workerPromise) {
                this.activeQueueWorkerPromise = null;
            }
        }
    }
    ...
    async _processUpsertJob(job) {
        ...
        try {
            ...
            materiallyChanged = await this._deleteFileUnlocked(normalizedPath);
            this._afterSuccessfulIngest(job.library);
            return materiallyChanged;
        }
        materiallyChanged = await this.upsertFile(normalizedPath);
        this._afterSuccessfulIngest(job.library);
        return materiallyChanged;
    }

    async _processDeleteJob(job) {
        const materiallyChanged = await this._deleteFileUnlocked(this._normalizeFilePath(job.abs_path));
        this._afterSuccessfulIngest(job.library);
        return materiallyChanged;
    }
    ...
    _markTextIndexDirty(library) {
        this.dirtyTextIndexLibraries.add(safeLibraryName(library));
    }

    async _rebuildDirtyTextIndexes(reason = 'lifecycle boundary') {
        const dirtyLibraries = Array.from(this.dirtyTextIndexLibraries);
        for (const library of dirtyLibraries) {
            try {
                await this._withLibraryQueue(library, async () => {
                    if (!this.dirtyTextIndexLibraries.has(library)) return;
                    const handle = this.getOrOpenLibrary(library);
                    this._beginLibraryUse(handle);
                    try {
                        if (typeof handle.db?.buildTextIndex !== 'function') {
                            throw new Error('TriviumDB buildTextIndex() is unavailable.');
                        }
                        handle.db.buildTextIndex();
                        this._safeFlush(handle.db);
                        this.dirtyTextIndexLibraries.delete(library);
                        console.log(`[TDBKnowledge] 🔤 Rebuilt text index for "${library}" at ${reason}.`);
                    } finally {
                        this._endLibraryUse(handle);
                    }
                });
            } catch (error) {
                console.warn(`[TDBKnowledge] ⚠️ Text-index rebuild failed for "${library}" at ${reason}; library remains dirty: ${error.message}`);
            }
        }
    }

    async upsertFile(filePath, options = {}) {
        if (!this._isIndexable(filePath)) return false;
        const normalizedPath = this._normalizeFilePath(filePath);
    ...
    async _upsertFileUnlocked(filePath, options = {}) {
        ...
        if (!this._isCurrentFileEvent(normalizedPath, eventVersion)) return false;
        ...
        try {
            ...
            this._markTextIndexDirty(library);
    ...
    async _deleteExistingFileNodes(handle, library, relPath) {
        ...
        const materiallyChanged = !!fileRow || chunkRows.length > 0;
    ...
    async deleteFile(filePath, options = {}) {
        const normalizedPath = this._normalizeFilePath(filePath);
        if (!this._isIndexable(normalizedPath)) return false;
        const { library } = this._resolveLibrary(normalizedPath);
    ...
    async _deleteFileUnlocked(filePath, options = {}) {
        ...
        this._beginLibraryUse(handle);
        try {
            const materiallyChanged = await this._deleteExistingFileNodes(handle, library, relPath);
            if (!materiallyChanged) return false;
            this._safeFlush(handle.db);
            this._markTextIndexDirty(library);
    ...
    async shutdown() {
        ...
        if (this.recovery) await this.recovery.shutdown();
        try {
            await this._runQueueWorker();
        } catch (error) {
            console.warn('[TDBKnowledge] Queue drain during shutdown failed:', error.message);
        }
        if (this.dirtyTextIndexLibraries.size > 0) {
            await this._rebuildDirtyTextIndexes('shutdown');
        }
