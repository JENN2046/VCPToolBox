...
} = require('./codexMemoryConstants');

const DEFAULT_AUDIT_WINDOW = 500;
const MAX_AUDIT_BYTES = 1024 * 1024;

const TARGET_TO_DB_NAME = {
    process: PROCESS_DIARY_NAME,
    knowledge: KNOWLEDGE_DIARY_NAME
};
...
function buildCodexAdaptiveProfiles({ recallEntries = [], writeEntries = [], config = DEFAULT_CODEX_ADAPTIVE_CONFIG }) {
    const diaries = {
        [PROCESS_DIARY_NAME]: createEmptyDiaryProfile(PROCESS_DIARY_NAME, 'process', config),
        [KNOWLEDGE_DIARY_NAME]: createEmptyDiaryProfile(KNOWLEDGE_DIARY_NAME, 'knowledge', config)
    };
    ...
    for (const dbName of Object.keys(diaries)) {
        ...
        if (config.enabled === false) {
            ...
            diary.reasons.push('Adaptive tuning disabled.');
            continue;
        }

        if (diary.writeCount < config.minWritesBeforeAdjust) {
            diary.status = 'warming';
            diary.reasons.push('Insufficient writes; keep warming before tuning.');
            continue;
        }
        ...
        if (hitGap <= 0 && lowScorePenalty <= 0) {
            diary.status = 'steady';
            diary.reasons.push('Hit rate is within target; keep current parameters.');
            continue;
        }
        ...
        diary.reasons.push(`Hit rate below target (${diary.hitRate.toFixed(2)} < ${config.targetHitRate.toFixed(2)}).`);
        if (lowScorePenalty > 0) {
            diary.reasons.push(`Average top score below threshold (${diary.avgTopScore.toFixed(3)} < ${config.lowScoreThreshold}).`);
        }
    }
...
function buildCodexTagContributionInsights({ recallEntries = [], config = DEFAULT_CODEX_ADAPTIVE_CONFIG }) {
    ...
    for (const entry of recallEntries) {
        const dbName = typeof entry.dbName === 'string' ? entry.dbName.trim() : '';
        const target = dbName === PROCESS_DIARY_NAME
            ? 'process'
    ...
    const byDiary = [PROCESS_DIARY_NAME, KNOWLEDGE_DIARY_NAME].map(dbName => ({
        dbName,
