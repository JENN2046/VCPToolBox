...
class LightMemoPlugin {
    ...
    async processToolCall(args) {
        try {
            const includeTrace = this._parseRiverTraceFlag(args.include_river_trace);
            if (includeTrace && this._isTagMemoABRequest(args)) {
                throw new Error('RIVER_TRACE_REQUIRES_SEARCH_RIVER_PROFILE');
            }
            if (Object.hasOwn(args, 'cold_text_mode')) {
                if (!['legacy_full_body', 'preview'].includes(args.cold_text_mode)) {
                    throw new Error('INVALID_COLD_TEXT_MODE');
                }
                if (this._isTagMemoABRequest(args)) {
                    throw new Error('COLD_TEXT_MODE_REQUIRES_COLD_SCOPE');
                }
            }
            if (args.cold_result_mode !== undefined && !['text', 'structured'].includes(args.cold_result_mode)) {
                throw new Error('INVALID_COLD_RESULT_MODE');
            }
            if (args.cold_result_mode === 'structured' && this._isTagMemoABRequest(args)) {
            ...
            const result = this._isTagMemoABRequest(args)
                ...
                : await this.handleSearch(args, { includeTrace });
    ...
    async handleSearch(args, { includeTrace = false } = {}) {
        ...
        if (includeTrace && (
            ...
            || this._parseRerankOptions(rerank).enabled
            || aiMemoOptions.enabled
        )) {
            throw new Error('RIVER_TRACE_REQUIRES_SEARCH_RIVER_PROFILE');
        }
        ...
        if (candidates.length === 0) {
            if (includeTrace) throw new Error('RIVER_TRACE_NO_CANDIDATES');
    ...
    async _handleRiverMemoSearch({
        ...
        // Observe the exact production result before projection/formatting; no second retrieval.
        if (includeTrace) {
            return {
     ...
     */
    async _handleColdKnowledgeSearch({ query, libraries, k, rerank, coldResultMode = 'text', coldTextMode = 'legacy_full_body' }) {
        ...
        const { useRerank, rrfOptions } = this._parseColdKnowledgeRerank(rerank);
        ...
        const normalizedK = Math.max(1, Math.floor(this._parseNumber(k, 5)));
        // Keep preview provenance separate from the unchanged text pipeline.
        const coldPreview = Symbol('coldPreview');
        const finish = (docs, text) => coldResultMode !== 'structured' ? text : {
            ...this._buildAiFriendlyTextResult(text).result,
            cold_result: {
                schema_version: 1,
                stage: 'post_tdb_topk',
                rerank_applied: false,
                query,
                requested_libraries: [...libraries], // [] means all Cold libraries.
                k: normalizedK,
                documents: docs.map((doc, index) => ({
                    rank: index + 1,
                    library: doc.dbName,
                    source_file: typeof doc.sourceFile === 'string' && doc.sourceFile
                        && !path.isAbsolute(doc.sourceFile) && !path.win32.isAbsolute(doc.sourceFile)
                        ? doc.sourceFile : null,
                    text_preview: doc[coldPreview].text,
                    vector_score: doc[coldPreview].score,
                    hybrid_score: doc[coldPreview].score
                }))
            }
        };
        ...
        try {
            hits = await this.tdbKnowledgeManager.search(query, {
                ...
                minScore: 0.1,
                hybridAlpha: 0.65,
                // Expand locally with a byte budget; TDB's legacy expand reads whole files.
                expand: false
            });
        } catch (error) {
        ...
        let docs = hits.map(h => ({
            ...
            ...(coldResultMode === 'structured' ? { [coldPreview]: {
                text: String(h.text || h.payload?.text_preview || ''),
                score: Number.isFinite(h.score) ? h.score : null
            } } : {})
        })).filter(d => d.text);
        docs = this._dedupeColdKnowledgeDocs(docs);
        if (coldTextMode === 'legacy_full_body') {
            docs = await this._expandColdKnowledgeDocs(docs);
        }
        ...
        return finish(docs, this._formatColdKnowledgeResults(docs, query, libraries, coldTextMode !== 'preview'));
    }

    _parseColdKnowledgeRerank(rerank) {
        ...
        return { useRerank, rrfOptions };
    }


    _dedupeColdKnowledgeDocs(docs) {
        // Retain the first ordered hit per library/source; missing sources stay independent.
        const seen = new Set();
    ...
    async _expandColdKnowledgeDocs(docs) {
        const limit = Math.min(12000, Math.floor(48000 / Math.max(1, docs.length)));
        const budget = { remaining: 1024 * 1024 }; // Actual bytes read per request.
        const configuredRoot = this.tdbKnowledgeManager.config?.rootPath;
        let root;
        try { if (configuredRoot) root = await fs.realpath(configuredRoot); } catch (_) { /* preview fallback */ }
        const out = [];
        for (const doc of docs) {
            const preview = String(doc.text || '');
            let text = preview, start = 0, expanded = false, reason;
            try {
                if (!root || !doc.sourceFile) throw new Error('源路径不可用');
                if (!budget.remaining) throw new Error('本次读取预算已用完');
                text = await this._readColdSource(root, configuredRoot, doc, budget);
                const anchor = preview.trim();
                const at = anchor ? text.indexOf(anchor) : -1;
                if (at < 0) throw new Error('命中预览未在源文件中找到，索引可能已过期');
                if (text.length > limit) {
                    if (text.indexOf(anchor, at + 1) !== -1) throw new Error('命中预览在源文件中重复，无法唯一定位');
                ...
                text = preview;
                start = 0;
                reason = error.coldBodyReason || (error.code ? '源文件读取失败' : error.message);
            }
            // Ranges use UTF-16 offsets in the parent (or in the fallback preview).
            if (start > 0 && /[\uDC00-\uDFFF]/.test(text[start])) start++;
            let end = Math.min(text.length, start + limit);
            if (end > start && /[\uD800-\uDBFF]/.test(text[end - 1])) end--;
            out.push({ ...doc, text: text.slice(start, end), bodyExpanded: expanded,
                bodyReason: reason, bodyTruncated: start > 0 || end < text.length,
                bodyRange: [start, end], bodyLength: text.length });
        }
        return out.filter(doc => doc.text);
    }

    async _readColdSource(root, configuredRoot, doc, budget) {
        const fail = message => { const error = new Error(message); error.coldBodyReason = message; throw error; };
        const inside = relative => relative && relative !== '..' && !relative.startsWith(`..${path.sep}`) && !path.isAbsolute(relative);
        if (path.isAbsolute(doc.sourceFile)) fail('源路径超出知识库范围');
        ...
        // Apply the manager's existing library naming rule to the resolved path.
        const scope = this.tdbKnowledgeManager._resolveLibrary(path.join(configuredRoot, relative));
    ...
    _formatScoreMetadata(result, vectorType) {
        let scoreValue = null;
        ...
        const display = 'raw=' + raw
            ...
            + '; field=' + scoreField
            + '; calibrated=false';
        return { value: scoreValue, type: scoreType, field: scoreField, display, metadata: display };
    }

    _formatColdKnowledgeResults(results, query, libraries, includeBodyStatus = true) {
        ...
        results.forEach((r) => {
            const score = this._formatScoreMetadata(r, 'TDB');
            ...
            if (includeBodyStatus) {
                const bodyState = r.bodyExpanded ? '父文档正文' : `预览；未确认取得父文档正文；${r.bodyReason || '正文不可用'}`;
                const range = r.bodyRange || [0, (r.text || '').length];
                const truncation = r.bodyTruncated ? `; 已截断，${r.bodyExpanded ? '源文' : '预览'}字符范围 [${range[0]}, ${range[1]}) / ${r.bodyLength}` : '';
    ...
    formatResults(results, query) {
        ...
        results.forEach((r, index) => {
            const score = this._formatScoreMetadata(r, '向量');
            const scoreDisplay = score.display;
