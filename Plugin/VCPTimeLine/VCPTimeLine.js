...
class VCPTimeLine {
    ...
    buildContentTextView(content) {
        const segments = [];
        if (typeof content === 'string') {
            segments.push({ text: content, start: 0 });
        } else if (Array.isArray(content)) {
            for (const part of content) {
                if (part?.type === 'text' && typeof part.text === 'string') {
                    segments.push({ text: part.text, start: 0 });
                }
            }
        } else if (content && typeof content.text === 'string') {
            segments.push({ text: content.text, start: 0 });
        }

        let cursor = 0;
        for (let index = 0; index < segments.length; index++) {
            segments[index].start = cursor;
            cursor += segments[index].text.length;
    ...
    parsePlaceholder(match) {
        ...
        if (values.length === 1) {
            ...
            if (/^\d+$/.test(raw) && numeric >= 1) {
                k = this.normalizePositiveInteger(numeric, k, 1, 100);
            }
            ...
            if (/^\d+$/.test(values[0]) && parsedK >= 1) {
                k = this.normalizePositiveInteger(parsedK, k, 1, 100);
            }
    ...
    async processMessages(messages) {
        ...
        // RAG/被动上下文块中的占位符只是记忆正文，不具备声明权限。
        // 先确定唯一声明，再以单次正则替换清除可信载体中的其余声明，
        // 避免注入内容被递归展开，同时不改写普通 user/assistant 原文。
        let declaration = null;
        for (let index = 0; index < messages.length && !declaration; index++) {
            const message = messages[index];
            const view = this.buildContentTextView(message?.content);
            const text = view.text;
            const trusted = message?.role === 'system'
                || (message?.role === 'user' && /^\s*\[系统/.test(text));
            if (!trusted) continue;

            const match = findMatchesOutsidePassiveBlocks(text, PLACEHOLDER_REGEX)[0];
            if (match) {
                declaration = {
                    index,
                    globalStart: match.index,
                    ...this.parsePlaceholder(match)
                };
            }
        }

        if (!declaration) return messages;

        const safeAgentName = this.safeAgentName(declaration.agentName);
        const queryContext = await this.buildQueryContext(messages);
        const injection = await this.buildInjection(
            safeAgentName,
            queryContext,
            declaration.k,
            declaration.threshold
        );
        const passiveInjection = buildPassiveBlock(injection, {
            source: 'VCPTimeLine',
        ...
        return messages.map((message, index) => {
            const view = this.buildContentTextView(message.content);
            const ranges = getPassiveBlockRanges(view.text);
            const trusted = message?.role === 'system'
                || (message?.role === 'user' && /^\s*\[系统/.test(view.text));
            let segmentIndex = 0;

            return {
                ...message,
                content: this.replaceText(message.content, text => {
                    const segment = view.segments[segmentIndex++] || { start: 0 };
                    return text.replace(
                        new RegExp(PLACEHOLDER_REGEX.source, PLACEHOLDER_REGEX.flags),
                        (fullMatch, _agentName, _firstValue, _secondValue, localStart) => {
                            const globalStart = segment.start + localStart;
                            const globalEnd = globalStart + fullMatch.length;
                            if (overlapsAnyRange(globalStart, globalEnd, ranges)) return fullMatch;
                            if (index === declaration.index && globalStart === declaration.globalStart) {
                                return passiveInjection;
                            }
                            return trusted ? '' : fullMatch;
                        }
                    );
                })
            };
    ...
    health() {
        return {
            status: 'ready',
            activeGenerationTasks: this.generationLocks.size
        };
    }

    getReloadBlockers() {
        return Array.from(this.generationLocks.keys()).map(agentName => ({
            type: 'active_timeline_generation',
            agentName,
            status: this.getGenerationStatus(agentName)
        }));
    }
    ...
    async shutdown() {
        if (this.generationLocks.size === 0) return;
        const deadline = Date.now() + 8000;
        while (this.generationLocks.size > 0 && Date.now() < deadline) {
            await Promise.race([
                Promise.allSettled(Array.from(this.generationLocks.values())),
                new Promise(resolve => setTimeout(resolve, 100))
            ]);
        }
        if (this.generationLocks.size > 0) {
            const error = new Error(
                `${this.generationLocks.size} VCPTimeLine generation task(s) are still active.`
            );
            error.code = 'ACTIVE_TIMELINE_TASKS';
            throw error;
        }
    }
