...
function updatePlaceholders() {
  if (!hostPluginManager?.staticPlaceholderValues) return;
  hostPluginManager.staticPlaceholderValues.set(STATUS_PLACEHOLDER, {
    value: buildStatusText(),
    serverId: 'local'
  });
  hostPluginManager.staticPlaceholderValues.set(RECENT_PLACEHOLDER, {
...
async function initialize(initialConfig = {}, dependencies = {}) {
  ...
  if (!hostPluginManager?.staticPlaceholderValues) {
    throw new Error('VCPQQBotServer requires an injected PluginManager placeholder store.');
  }
...
async function shutdown() {
  ...
  if (ws) {
    ...
    await new Promise(resolve => {
      ...
      timer.unref?.();
      if (socket.readyState === WebSocket.CLOSED) return finish();
