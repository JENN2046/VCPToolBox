...
function replaceLastMemoInContent(content, triggerText, replacement) {
    const replaceLast = text => replaceLastLiteralOutsidePassiveBlocks(text, triggerText, replacement);
...
function injectMemo(messages) {
    ...
    for (let index = 0; index < messages.length; index++) {
        ...
        const matches = findMatchesOutsidePassiveBlocks(text, MEMO_TRIGGER_REGEX);
