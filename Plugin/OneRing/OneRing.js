...
function getLastTriggerMatch(systemText) {
    ...
    const matches = [...stripPassiveBlocks(systemText).matchAll(TRIGGER_GLOBAL_REGEX)];
...
function getLastOnlyTriggerMatch(systemText) {
    if (typeof systemText !== 'string') return null;
    const matches = [...stripPassiveBlocks(systemText).matchAll(ONLY_TRIGGER_GLOBAL_REGEX)];
    if (matches.length === 0) return null;
    return matches[matches.length - 1];
}
...
function getLastNoticeMeta(systemText) {
    if (typeof systemText !== 'string') return null;
    systemText = stripPassiveBlocks(systemText);
...
function replaceTriggerWithNotice(content, triggerText, agentName, frontendSource, mode) {
    ...
    if (typeof content === 'string') {
        return replaceLastLiteralOutsidePassiveBlocks(content, triggerText, notice);
    }
    ...
    if (Array.isArray(content)) {
        ...
        for (let i = result.length - 1; i >= 0; i--) {
            ...
            if (
                ...
                result[i] = { ...part, text: replaceLastLiteralOutsidePassiveBlocks(part.text, triggerText, notice) };
                return result;
            }
        }
        return result;
    ...
    if (content && typeof content === 'object' && typeof content.text === 'string') {
        return { ...content, text: replaceLastLiteralOutsidePassiveBlocks(content.text, triggerText, notice) };
    }
...
class OneRingPreprocessor {
    ...
    async shutdown() {
        if (hotConfigWatcher) {
            await hotConfigWatcher.close().catch(() => {});
            hotConfigWatcher = null;
        }
        fuzzyPool.close();
        db.closeAll();
