...
const PLUGIN_NAME = 'generate_delivery_queue_schedule';
...
async function processToolCall(args) {
  try {
    const data = service.generateDeliveryQueueSchedule(args || {});
    return _success(data, {
      entity: 'delivery_queue_schedule',
      sync_mode: 'local_shadow',
      matched_records: data.summary.total_records,
      actionable_records: data.summary.actionable_records
    });
  } catch (error) {
    return _error(
      error.code || 'INTERNAL_ERROR',
      error.message || 'Failed to generate delivery queue schedule.',
      error.field || null,
      error.details || {}
    );
  }
