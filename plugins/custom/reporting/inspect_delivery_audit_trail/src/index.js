...
const PLUGIN_NAME = 'inspect_delivery_audit_trail';
...
async function processToolCall(args) {
  try {
    const data = service.inspectDeliveryAuditTrail(args || {});
    return _success(data, {
      entity: 'delivery_audit_trail',
      sync_mode: 'local_shadow',
      matched_records: data.audit_summary.total_records,
      total_events: data.audit_summary.total_events
    });
  } catch (error) {
    return _error(
      error.code || 'INTERNAL_ERROR',
      error.message || 'Failed to inspect delivery audit trail.',
      error.field || null,
      error.details || {}
    );
  }
