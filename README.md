...
- **DingTalkTable**：钉钉 AI 表格写入器（新增）- 将日报/周报写入钉钉 AI 表格，支持 MCP 协议调用
- **WorkLogScheduler**：工作日志调度器（新增）- 定时触发日报/周报生成，读取工作记录并推送提醒
- **WeeklyReportGenerator**：周报生成器（新增）- 从工作记录自动生成结构化周报，支持导出到钉钉表格
  - 完全为 AI 设计
  - 有强大的镜像、纠错、回退和兜底功能
  - 拥有近乎完备的资源管理器 API 能力
  - 可以处理大量复格式文档和多模态文件
