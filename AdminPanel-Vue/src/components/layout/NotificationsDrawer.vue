      ...
      >
        ...
        <section class="notifications-drawer__list" aria-live="polite">
          ...
          >
            ...
            <section
              v-if="item.toolApproval?.authorityTarget"
              class="notification-authority-target"
              aria-label="本次审批的确切授权目标"
            >
              <strong>操作：{{ item.toolApproval.authorityTarget.operation }}</strong>
              <dl>
                <template v-for="field in item.toolApproval.authorityTarget.fields" :key="field.label">
                  <dt>{{ field.label }}</dt>
                  <dd>{{ field.value || "缺失或无效" }}</dd>
                </template>
              </dl>
              <p v-if="!item.toolApproval.authorityTarget.valid" role="alert">
                无效的授权审批请求：目标信息缺失或用途不受支持，无法允许。你仍可拒绝此请求。
              </p>
            </section>

            <div v-if="item.toolApproval" class="notification-approval">
              <p v-if="!store.approvalReady && !item.toolApproval.hostTerminal">审批通道尚未同步，操作已暂停。</p>
              <button v-if="!store.approvalReady && !item.toolApproval.hostTerminal" type="button" @click="store.syncApprovals()">{{ store.approvalNeedsReconnect ? "重新连接并同步" : "同步审批状态" }}</button>
              <p role="status" aria-live="polite" class="notification-approval__status">{{ store.approvalMessage(item.toolApproval) }}</p>
              <textarea
                v-model="approvalReasons[item.toolApproval.requestId]"
                class="notification-approval__reason"
                :disabled="!store.canRespond(item.toolApproval, false)"
                rows="3"
                maxlength="1000"
              ...
              <div class="notification-approval__actions">
                <button
                  ...
                  :disabled="!store.canRespond(item.toolApproval, true)"
...
watch(() => store.items.map(item => item.toolApproval), records => {
  for (const approval of records) if (approval && approval.state !== "ACTIVE") delete approvalReasons[approval.requestId];
}, { deep: true });
...
function finishApproval(item: NotificationItem, approved: boolean) {
  if (approved && item.toolApproval?.authorityTarget?.valid === false) return;
  ...
  if (!sent) {
    showMessage("审批尚未获服务器确认，请查看卡片状态。", "error");
    return;
  }

  delete approvalReasons[requestId];
  // Sending is not Host acceptance; the card remains until Host evidence arrives.
}
</script>
...
.notification-authority-target dl {
  margin: 8px 0 0;
}

.notification-authority-target dt {
  margin-top: 8px;
  color: var(--secondary-text);
}

.notification-authority-target dd {
  margin: 4px 0 0;
...
.notification-approval__btn:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
