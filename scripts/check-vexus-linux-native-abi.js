...
assertFunctionSurface(
  nativeModule,
  ['NativeKnowledgeRuntime', 'VexusIndex', 'VexusWatcher'],
  'native module'
);
assertFunctionSurface(nativeModule.VexusIndex, ['load'], 'VexusIndex static');
assertFunctionSurface(
  nativeModule.VexusIndex.prototype,
  ['add', 'remove', 'save', 'search', 'stats'],
  'VexusIndex'
);
...
try {
  ...
  if (!firstStats || firstStats.totalVectors < 2) {
    throw new Error('VexusIndex stats smoke failed');
  }
