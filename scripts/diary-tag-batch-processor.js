...
async function generateTagsWithAI(content, maxRetries = 3) {
    ...
    const promptFilePath = path.isAbsolute(TAG_MODEL_PROMPT_FILE)
        ? TAG_MODEL_PROMPT_FILE
        : path.join(repoRoot, TAG_MODEL_PROMPT_FILE);
    let systemPrompt;
    try {
...
async function main() {
    ...
    if (!targetPath) {
        console.log('使用方法：');
        console.log('  node scripts/diary-tag-batch-processor.js [目标文件夹路径]\n');
        console.log('示例：');
        console.log('  node scripts/diary-tag-batch-processor.js ./dailynote');
        console.log('  node scripts/diary-tag-batch-processor.js "C:\\Users\\Admin\\Documents\\旧日记"\n');
        process.exit(1);
    }
