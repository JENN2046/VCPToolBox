...
function formatRuntimeEnvDebugKeyList(env = {}) {
    ...
    const visibleKeys = keys.filter(key => !isPluginRuntimeEnvKeyDenied(key)).sort();
...
class PluginManager extends EventEmitter {
    ...
    async _discoverLegacyPluginManifestsFromDir(pluginRoot, sourceLabel = 'core', rootInfo = null) {
        ...
        for (const record of result.records || []) {
            ...
            const nativeManifest = record.manifest;
    ...
    async _executeStaticPluginCommand(plugin) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const pluginProcess = this._spawnPluginProcess(command, args, {
                cwd: plugin.basePath,
                shell: true,
                env: envForProcess,
                windowsHide: true,
    ...
    async executePlugin(pluginName, inputData, requestIp = null, executionOptions = {}) {
        ...
        if (executionOptions && typeof executionOptions === 'object') {
            if (executionOptions.requestSource) additionalEnv.VCP_REQUEST_SOURCE = executionOptions.requestSource;
            if (executionOptions.agentAlias) additionalEnv.VCP_AGENT_ALIAS = executionOptions.agentAlias;
            if (executionOptions.agentId) additionalEnv.VCP_AGENT_ID = executionOptions.agentId;
            if (executionOptions.executionContext) additionalEnv.VCP_EXECUTION_CONTEXT = executionOptions.executionContext;
        }
    ...
    startPluginWatcher() {
        ...
        console.log(`[PluginManager] Chokidar is now watching ${watchRoots.map(rootPath => this._formatPluginEventPathForLog(rootPath)).join(', ')} (depth: 1, strict manifest filter).`);
        return this.pluginWatcher;
    }
    ...
    async _refreshPluginManifestMetadata(filePath) {
        ...
        if (runtimeChanged) {
            ...
            refreshedManifest.pluginSource = currentManifest.pluginSource;
            refreshedManifest.pluginRoot = currentManifest.pluginRoot;
            refreshedManifest.pluginRootId = currentManifest.pluginRootId;
            refreshedManifest.pluginRootDisplayPath = currentManifest.pluginRootDisplayPath;
            this.plugins.set(freshManifest.name, refreshedManifest);
        }
