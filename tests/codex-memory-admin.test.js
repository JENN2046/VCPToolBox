...
test('codex-memory admin overview should include recall audit summary and recent hits', async () => {
    ...
    await fs.writeFile(recallLogPath, [
        JSON.stringify({
            ...
            coreTags: ['codex'],
            topSourceFile: 'Codex/2026-04-13-00_00_01.txt',
            memoryIds: ['codex-process-alpha'],
            sourceFiles: ['Codex/2026-04-13-00_00_01.txt'],
            fromCache: false,
            sourceKinds: ['rag']
        }),
        JSON.stringify({
    ...
    try {
        ...
        assert.equal(data.recall.recent[1].topSourceFile, 'Codex/2026-04-13-00_00_01.txt');
        assert.equal(data.adaptive.config.enabled, true);
        assert.equal(data.adaptive.profiles.length, 2);
        assert.equal(data.adaptive.profiles[0].dbName, 'Codex');
