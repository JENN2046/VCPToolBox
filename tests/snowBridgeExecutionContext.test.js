...
test('SnowBridge tags bridged tool calls with snowbridge requestSource', () => {
	...
	assert.match(methodSource, /pluginManager\.processToolCall\(\s*toolName,\s*toolArgs,\s*null,\s*\{\s*requestSource:\s*'snowbridge'\s*\},?\s*\)/);
});
