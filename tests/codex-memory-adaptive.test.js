...
test('buildCodexAdaptiveProfiles should boost Codex diary when hit rate is below target', () => {
    const result = buildCodexAdaptiveProfiles({
        ...
        recallEntries: [
            { dbName: PROCESS_DIARY_NAME, recallType: 'snippet', topScore: 0.61, fromCache: false, timestamp: '2026-04-13T00:10:00.000Z' }
        ]
    });

    const codexProfile = result.profiles.find(item => item.dbName === PROCESS_DIARY_NAME);
    const knowledgeProfile = result.profiles.find(item => item.dbName === KNOWLEDGE_DIARY_NAME);
