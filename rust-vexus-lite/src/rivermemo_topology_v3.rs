...
struct AnchorOutput {
    contacts_trace: Option<Vec<Value>>,
...
fn compute_anchors(
    ...
    contacts
        ...
        .map(|candidate_contacts| {
            let mut contacts_trace = include_trace.then(Vec::new);
            ...
            for contact in &candidate_contacts {
                ...
                if let Some(trace) = contacts_trace.as_mut() {
                    trace.push(serde_json::json!({
                        "seedId": contact.0, "candidateTagId": contact.1,
                        "exact": contact.2, "similarity": contact.3,
                        "seedMass": contact.4, "chunkCosine": contact.5,
                        "inbound": inbound, "maxInbound": artifact.max_inbound,
                        "specificity": specificity, "specificityFloor": config.specificity_floor,
                        "poolCount": pool_counts.get(&contact.0), "poolSize": curves.len(),
                        "rarity": rarity, "rarityFloor": config.rarity_floor,
                        "maxSeedMass": max_seed_mass, "normalizedMass": normalized_mass,
                        "matchWeight": match_weight,
                        "semanticAnchorThreshold": config.semantic_anchor_threshold,
                        "contribution": contribution,
                        "noContactProbabilityAfter": no_contact_probability
                    }));
                }
            }
...
fn assign_v3_scores(
    ...
    let strongest_candidate_id = if include_trace {
        strongest.map(|entry| work[entry.0].curve.id)
    } else { None };

    for (index, item) in work.iter_mut().enumerate() {
        let mut activation_stage = "below_or_equal_threshold";
        let mut threshold_passed = false;
        let mut normalized_activation = None;
        let activation = if item.anchor.strength <= threshold {
            ...
            if include_trace {
                activation_stage = "smoothstep";
                threshold_passed = true;
                normalized_activation = Some(normalized);
            }
            normalized * normalized * (3.0 - 2.0 * normalized)
        } else {
            if include_trace {
                activation_stage = "degenerate_saturation";
                threshold_passed = true;
            }
            1.0
        };
        ...
        if include_trace {
            item.anchor.decision_trace = Some(serde_json::json!({
                "strength": item.anchor.strength,
                "anchorMean": anchor_mean, "anchorVariance": anchor_variance,
                "anchorStd": anchor_std, "poolSize": ranked_anchors.len(),
                "activationFloor": config.anchor_activation_floor,
                "activationZ": config.anchor_activation_z, "threshold": threshold,
                "saturation": config.anchor_saturation,
                "stage": activation_stage, "normalizedActivation": normalized_activation,
                "thresholdPassed": threshold_passed,
                "activation": activation, "bonusCap": config.anchor_bonus_cap,
                "awardedBonus": item.anchor_bonus,
                "fallbackReliabilityCap": config.fallback_reliability_cap,
                "reliabilitySeedSaturation": config.reliability_seed_saturation,
                "frontier": {
                    "strongestWorkIndexAndStrength": strongest,
                    "strongestCandidateId": strongest_candidate_id,
                    "secondStrength": second, "promote": promote,
                    "absoluteFloor": config.anchor_frontier_abs_floor,
                    "contrast": config.anchor_frontier_contrast,
                    "workIndex": index, "finalRole": item.role
                }
            }));
        }
    }
...
pub(crate) fn run_native(
    ...
    assign_v3_scores(&mut work, mode, &omega, &input.config, input.include_trace);
    // Keep the complete selected pool: threshold/rarity depend on candidates
    // beyond topK too. Identities and computed diagnostics only, never content.
    let anchor_pool_trace = input.include_trace.then(|| work.iter().enumerate().map(|(index, item)| {
        serde_json::json!({
            "workIndex": index, "chunkId": item.curve.id, "fileId": item.curve.file_id,
            "tags": item.curve.tags.iter().map(|tag| serde_json::json!({
                "id": tag.id, "name": tag.name
            })).collect::<Vec<_>>(),
            "contacts": item.anchor.contacts_trace,
            "anchorScore": item.anchor.score, "anchorReliability": item.anchor.reliability,
            "anchorStrength": item.anchor.strength, "meanClosure": item.anchor.mean_closure,
            "anchorDecision": item.anchor.decision_trace
        })
    }).collect::<Vec<_>>());
    ...
    let output = NativeOutput {
        anchor_query: input.include_trace.then(|| serde_json::json!({
            "schema": "river-anchor-query-causality-v1",
            "observationHandle": input.observation_handle,
            "sourceField": input.query_state.source_field,
            "fieldProvenance": input.query_state.field_provenance.iter().map(|entry|
                serde_json::json!({"id": entry.id, "hop": entry.hop, "sourceType": entry.source_type})
            ).collect::<Vec<_>>(),
            "seeds": seeds,
            "candidatePool": anchor_pool_trace,
            "fallbackAnchor": fallback_anchor,
            "completeObservation": input.query_state.complete_observation
        })),
