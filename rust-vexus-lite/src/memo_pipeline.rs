...
struct PipelineMetadataOutput {
    #[serde(skip_serializing_if = "Option::is_none")]
    causality_trace: Option<serde_json::Value>,
    schema: String,
...
fn run_pipeline(
    ...
    // Preserve actual computed identities/scalars only; no vectors or new sensing.
    let causality_trace = input.include_trace.then(|| serde_json::json!({
        "schema": "memo-query-causality-v1",
        "epa": epa,
        "pyramid": pyramid,
        "coreTags": input.core_tags,
        "gatedTags": gated_tags.iter().map(|tag| serde_json::json!({
            "id": tag.id, "name": tag.name, "weight": tag.weight, "isCore": tag.is_core
        })).collect::<Vec<_>>(),
        "observation": observation,
        "selectedTags": selected_tags.iter().map(|tag| serde_json::json!({
            "id": tag.id, "name": tag.name, "weight": tag.weight, "isCore": tag.is_core
        })).collect::<Vec<_>>(),
        "effectiveTagBoost": effective_tag_boost,
        "dynamicCoreBoost": dynamic_core_boost
    }));
