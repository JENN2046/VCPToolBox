...
当前里程碑：Jenn fork 内长期维护收口（M13 Agent shadow validation pass；M8/S25 deferred）
...
| --- | --- | --- | --- |
| Agent | `VCP_AGENT_DIRS`、`VCP_AGENT_OVERRIDE_DIRS` | Jenn Agent 和 AgentOverrides | Shadow validation 已通过：external package checksum / path scan / env-unset proof clean；下一步才是 test-first loader contract implementation，不自动启用 runtime。 |
