...
```text
M9-M16：已完成 Agent 外置从 taskbook、copy-first、shadow validation 到 default-off runtime wiring。
M17：只有 env-on shadow / rollback drill PASS 后，才可进入 Agent 领域最终收口。
...
| --- | --- | --- | --- |
| Agent | `VCP_AGENT_ALLOWED_ROOTS`、`VCP_AGENT_DIRS`、`VCP_AGENT_OVERRIDE_DIRS` | Jenn Agent 和 AgentOverrides | 正式路线 M9-M18；当前 M16 PASS，下一步 M17 env-on shadow / rollback drill，仍不修改真实 `.env` 或读取 LocalState。 |
