...
当前里程碑：AdminPanel page/API route/nav decision taskbook（M95 PASS；M96 static metadata surface or closeout decision next）
...
- 长期路线图：正式 milestone，从原始 M0-M8 到 Jenn fork maintenance overlay M9-M95。
- 短期执行记录：实际 sprint ledger，记录 S1-S48 与 S50-S116 已完成工作；S49 upstream PR gate deferred。
...
- M95 完成 AdminPanel page/API route/nav decision taskbook；确认 5 个 page/API core static fallback 已存在，external metadata 仍默认关闭且非执行；记录 `ChannelHub` metadata/core route-id alignment guard；选择 future M96 只能做静态只读 metadata surface 或 closeout decision，不启 dynamic frontend runtime。
...
M95 PASS：1.0 / 1 unit（AdminPanel page/API route/nav decision taskbook；M96 static metadata surface or closeout decision next；ChannelHub route-id alignment guard recorded）
...
M95：AdminPanel page/API route/nav decision taskbook PASS；确认 core static fallback route/component entries 已存在，external metadata 保持默认关闭且非执行；记录 `ChannelHub` metadata `channel-hub` vs core `channel-hub-manager` alignment guard；future M96 只能在静态只读 metadata surface 或 closeout decision 内选择。
```
...
| AdminPanel page/API route/nav decision taskbook | M95 / S116 | document current core route/nav fallback reality, select future M96 static metadata surface or closeout decision, and keep dynamic runtime blocked | PASS；docs-only; core fallback retained; route-id alignment guard recorded; no frontend source/runtime/env/build/dist。 |
...
| Q77 | M95/S116 | PASS | AdminPanel | `CLEAN_UPSTREAM_CORE_JENN_EXTERNAL_RUNTIME_M95_ADMINPANEL_PAGE_API_ROUTE_NAV_DECISION_TASKBOOK_20260622.md`；core static route/nav fallback retained; external metadata remains non-executable; `ChannelHub` route-id alignment guard recorded; future M96 limited to static metadata surface or closeout decision。 |

### 5.5 M19-M95 Specific Step Plan
...
| AdminPanel page/API route/nav decision taskbook | M95 taskbook；core static route/nav fallback reality recorded; `ChannelHub` route-id alignment guard recorded; future M96 limited to static metadata surface or closeout decision | PASS_TASKBOOK_ONLY；dynamic frontend runtime, real env, build/dist, provider/OAuth actions still deferred |
