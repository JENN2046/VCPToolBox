...
当前里程碑：Jenn fork 内长期维护收口（M10 Agent source scan + skeleton ready；M8/S25 deferred）
...
| --- | --- | --- | --- |
| Agent | `VCP_AGENT_DIRS`、`VCP_AGENT_OVERRIDE_DIRS` | Jenn Agent 和 AgentOverrides | Source path scan 和 external skeleton 已完成；下一步才是 reviewed Agent content copy-first，不自动启用 runtime。 |
