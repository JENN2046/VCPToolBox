...
Progress: [##########] 97% (41.7 / 43 global milestone units; scope expanded by M42 PASS)

Last updated: 2026-06-21

当前里程碑：AgentOverrides local read smoke（M42 PASS；no production server）
...
- 长期路线图：正式 milestone，从原始 M0-M8 到 Jenn fork maintenance overlay M9-M42。
- 短期执行记录：实际 sprint ledger，记录 S1-S48 与 S50-S63 已完成工作；S49 upstream PR gate deferred。
...
- 全局 Progress 覆盖 M0-M42 全路线，只保留这一种进度口径。
- 每个 milestone 记 1 个 global milestone unit；M0-M42 合计 43 units。
- `PASS` 计 1 unit。
...
M0-M8 是原始 acceptance plan 阶段；M9-M42 是当前 Jenn fork 长期维护与本地稳定验收路线。两者共同计入顶部全局 Progress；原始 100 分仅作为历史验收拆分背景，不再单独维护进度。
...
Global Progress：41.7 / 43 = 96.98%，顶部显示为 97%
```
...
### 5.5 M19-M42 Specific Step Plan
...
| --- | --- | --- | --- |
| Agent | `VCP_AGENT_ALLOWED_ROOTS`、`VCP_AGENT_DIRS`、`VCP_AGENT_OVERRIDE_DIRS` | Jenn Agent 和 AgentOverrides | M41 已按授权在真实 `config.env` 启用 `AgentOverrides` only；M42 local read smoke PASS；`VCP_AGENT_DIRS` additive 暂不启用；core fallback 保留。 |
