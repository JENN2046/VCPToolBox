...
当前里程碑：Jenn fork 内长期维护收口（M14 Agent loader contract tests pass；M8/S25 deferred）
...
| --- | --- | --- | --- |
| Agent | `VCP_AGENT_ALLOWED_ROOTS`、`VCP_AGENT_DIRS`、`VCP_AGENT_OVERRIDE_DIRS` | Jenn Agent 和 AgentOverrides | Pure resolver contract tests 已通过；下一步才是 AgentManager runtime wiring design / review，不自动启用 runtime。 |
