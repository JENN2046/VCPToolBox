...
当前里程碑：Jenn fork 内长期维护收口（M15 AgentManager wiring review taskbook ready；M8/S25 deferred）
...
当前 sprint 已展开到 Jenn fork 内长期维护收口 / Agent externalization。M8 upstream PR 仍按用户决定 deferred，不自动打开 upstream PR。
...
| --- | --- | --- | --- |
| Agent | `VCP_AGENT_ALLOWED_ROOTS`、`VCP_AGENT_DIRS`、`VCP_AGENT_OVERRIDE_DIRS` | Jenn Agent 和 AgentOverrides | AgentManager wiring review taskbook 已准备；下一步若继续是 runtime wiring patch，但必须先解决 alias / AdminPanel write policy，不自动启用 runtime。 |
