...
当前里程碑：Jenn fork 内长期维护收口（M11 Agent content gate pass；M8/S25 deferred）
...
| --- | --- | --- | --- |
| Agent | `VCP_AGENT_DIRS`、`VCP_AGENT_OVERRIDE_DIRS` | Jenn Agent 和 AgentOverrides | Candidate content gate 已通过；下一步才是 Agent content copy-first，不自动启用 runtime。 |
