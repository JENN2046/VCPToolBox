...
当前里程碑：Jenn fork 内长期维护收口（M12 Agent content copy-first pass；M8/S25 deferred）
...
| --- | --- | --- | --- |
| Agent | `VCP_AGENT_DIRS`、`VCP_AGENT_OVERRIDE_DIRS` | Jenn Agent 和 AgentOverrides | Content copy-first 已完成：7 additive + 2 override 进入 external package；下一步才是 shadow/loader contract validation，不自动启用 runtime。 |
