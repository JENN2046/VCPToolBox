...
当前里程碑：Jenn fork 内长期维护收口（M16 AgentManager default-off runtime wiring pass；M8/S25 deferred）
...
| --- | --- | --- | --- |
| Agent | `VCP_AGENT_ALLOWED_ROOTS`、`VCP_AGENT_DIRS`、`VCP_AGENT_OVERRIDE_DIRS` | Jenn Agent 和 AgentOverrides | AgentManager default-off runtime wiring 已通过；下一步才是 env-on shadow/rollback drill，仍不修改真实 `.env` 或读取 LocalState。 |
