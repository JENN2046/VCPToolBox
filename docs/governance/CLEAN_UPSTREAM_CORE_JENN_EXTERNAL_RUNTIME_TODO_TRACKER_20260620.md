...
Progress: [######----] 59% (17.7 / 30 global milestone units)

Last updated: 2026-06-21

当前里程碑：Jenn fork 内长期维护收口（M17 Agent env-on shadow / rollback drill pass；next M18 Agent domain final closeout decision；M8/S25 deferred；future domains queued）
...
- 短期执行记录：实际 sprint ledger，记录 S1-S38 已完成工作和 S39-S50 计划/延期项。
...
M9-M17 PASS：9.0 / 9 units
M18-M29 TODO / DEFERRED：0 / 12 units
Global Progress：17.7 / 30 = 59.0%，显示为 59%
```
...
```text
M9-M17：已完成 Agent 外置从 taskbook、copy-first、shadow validation、default-off runtime wiring 到 env-on shadow / rollback drill。
M17：PASS 后才可进入 Agent 领域最终收口。
M18：只做决策包和完成定义，不自动 delete/untrack/stub core Agent 文件。
...
| Agent 分域验收 | M9-M18 / S29-S39 | taskbook、source scan、candidate gate、copy-first、shadow、resolver、default-off wiring、env-on rollback、final decision | M9-M17 PASS；M18 TODO。 |
...
| M17-08 | PASS | 更新 tracker：M17/S38 PASS 或 BLOCK | M17/S38 PASS；M18 remains TODO | 不改 M18 为 PASS |
...
| --- | --- | --- | --- |
| Agent | `VCP_AGENT_ALLOWED_ROOTS`、`VCP_AGENT_DIRS`、`VCP_AGENT_OVERRIDE_DIRS` | Jenn Agent 和 AgentOverrides | 正式路线 M9-M18；当前 M17 PASS，下一步 M18 Agent domain final closeout decision，仍不修改真实 `.env` 或读取 LocalState。 |
